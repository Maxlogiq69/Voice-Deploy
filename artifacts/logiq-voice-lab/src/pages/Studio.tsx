...
export function Studio() {
  ...
  const { play, pause, resume, stop, speechState, progress } = useSpeech();

  const handleGenerate = () => {
    if (!script.trim()) return;
    play(script, selectedVoice, speed, pitch);
  };
    ...
    <div className="min-h-[100dvh] flex flex-col bg-background selection:bg-primary/20">
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 md:px-8 py-8 flex flex-col gap-8">
        <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <p className="text-muted-foreground mb-6">Write or paste your history content here. We'll handle the narration.</p>
